...
float4 PSmain(ScreenQuadOutput pin) : SV_TARGET
{
	...
	Texture2D<float4> texTexCoords = ResourceDescriptorHeap[GBufferIndices.TexCoordsIndex];
	...
	float2 texCoords = texTexCoords.Load(int3(position, 0)).xy;
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		float attenuation = 1.0f / pow(distance, 3.0f);
		...
		Lo += Lights.Light[i].Visibility * (((baseColor.rgb / PI) * kD) + specular) * radiance * NdotL;
	}
	...
	float3 directional = float3(0.0f, 0.0f, 0.0f);
	{
		...
		directional += (((baseColor.rgb / PI) * kD) + specular) * radiance * NdotL;
