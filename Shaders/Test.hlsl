...
ConstantBuffer<Indices> texIndices : register(b2, space0);
...
{
	float4 output = float4(pin.Normal, 1.0f);
	
	if (texIndices.BaseColor > -1)
	{
		Texture2D<float4> tex = ResourceDescriptorHeap[texIndices.BaseColor];
